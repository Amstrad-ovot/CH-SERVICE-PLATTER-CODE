...
#         data["age_reg_days"] = (data["today_date"] - data["call_date"]).dt.days
#         data["age_update_days"] = calculate_working_days_age(
#             data["status_updated_date"], 
#             data["today_date"], 
#             holidays
#         )
#         # data.to_excel("Aged_data.xlsx", index = False)
#         show_popup("main data is ready..",type= "success")
#         return data
#     except Exception as e:
#         print(f"Error in main_func is : {e}")
#         show_popup(f"Error in main_func is: {e}", type= "error")
...
def main_func(raw_file):
    try:
        ...
        data["age_reg_days"] = (todayDate - data["call_date"]).dt.days
        data["age_update_days"] = (todayDate - data["status_updated_date"]).dt.days
        data.to_excel("Ageing.xlsx", index= False)
        show_popup("main data is ready..",type= "success")
        return data
    except Exception as e:
        print(f"Error in main_func is : {e}")
...
def callAgewise_platter(raw_file):
    try:
        merged_data = main_func(raw_file)
        ...
        df_calc['Red Call 15+'] = np.where(is_above_14 & (merged_data["age_reg_days"] > 15), 1, 0)
        # Encroaching if age_reg_days == 15
        df_calc['Encroaching 15+'] = np.where(is_above_14 & (merged_data["age_reg_days"] == 15), 1, 0)
        ...
        show_popup("Successfully uploaded the Circle-wise Agewise report with the new age-day rules!", type = "success")
        return final_report
...
def fetch_and_format_report(uploaded_file):
    try:
        ...
        cicle_callagewise_summ = callAgewise_platter(uploaded_file)
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            # --- Sheet3: Circle CallAgewise platter ---
            if not cicle_callagewise_summ.empty:
                sheet_name = "Circle CallAgewise Platter"
                cicle_callagewise_summ.iloc[:, 1:] = cicle_callagewise_summ.iloc[:, 1:].apply(pd.to_numeric, errors='coerce').fillna(0).astype(int)
                cicle_callagewise_summ.to_excel(writer, sheet_name=sheet_name, index=False, startrow=1)
